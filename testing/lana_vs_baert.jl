using TensorToolbox
using BenchmarkTools
using Random
using Plots
using Statistics
using Printf
using LinearAlgebra

include("../algorithms/cyclicShift.jl")
include("../utility/tensor.jl")

# ============================================================
# 1. Methods: our CyclicShift variants vs TensorToolbox.ttm
# ============================================================

# Wrappers so all methods share the same (X, A, modes) signature.
cyclic_unity(X, A, modes)  = multiply_with_unity_fill(X, A, modes)
cyclic_permute(X, A, modes) = CyclicShiftMultiplication(X, A)

function ttm_toolbox(X, A, modes)
    M = MatrixCell(undef, length(A))
    for i in eachindex(A)
        M[i] = Matrix(A[i])
    end
    return ttm(X, M, collect(modes), 'n')
end

const METHODS = Dict{Symbol,Function}(
    :CyclicShift => cyclic_permute,
    :TensorToolboxTTM   => ttm_toolbox,
)

# ============================================================
# 2. Case generator (all modes active, same shape style as comparingMethods)
# ============================================================

function make_case(order::Symbol, n::Int, d::Int)
    X = randn(ntuple(_ -> n, d))

    idx = collect(1:d)
    sizes = if order === :normal
        idx .* n
    elseif order === :shuffle
        shuffle(idx) .* n
    elseif order === :reverse
        reverse(idx) .* n
    else
        error("Unknown order: $order")
    end

    @printf("d = %d | order = %s | sizes = %s\n",
            d, String(order), string(sizes))

    A = [randn(s, n) for s in sizes]
    modes = collect(1:d)
    return X, A, modes
end

# ============================================================
# 3. Result structs
# ============================================================

struct MethodStats
    time_ms::Float64
    memory::Float64
    allocs::Int
end

struct BenchmarkResults
    dims::Vector{Int}
    orders::Vector{Symbol}
    methods::Vector{Symbol}
    times::Dict{Symbol,Dict{Symbol,Vector{Float64}}}
    mem::Dict{Symbol,Dict{Symbol,Vector{Float64}}}
    allocs::Dict{Symbol,Dict{Symbol,Vector{Int}}}
end

# ============================================================
# 4. Benchmark one case
# ============================================================

function benchmark_case(order_sym::Symbol, n::Int, d::Int; methods = METHODS)
    X, A, modes = make_case(order_sym, n, d)
    results = Dict{Symbol,MethodStats}()

    println("  Methods for order = $(order_sym):")
    for (mname, f) in methods
        t = @benchmark $f($X, $A, $modes)
        time_ms = median(t.times) / 1e6
        mem     = t.memory
        allocs  = t.allocs

        @printf("    %-20s: time = %.3f ms | mem = %.2f MB | allocs = %d\n",
                String(mname), time_ms, mem / 1e6, allocs)

        results[mname] = MethodStats(time_ms, mem, allocs)
    end

    return results
end

# ============================================================
# 5. Run experiments across orders d
# ============================================================

function run_experiments(n::Int, dims::AbstractVector{<:Int}; methods = METHODS)
    orders = [:shuffle]

    times  = Dict(m => Dict(o => Float64[] for o in orders) for m in keys(methods))
    mems   = Dict(m => Dict(o => Float64[] for o in orders) for m in keys(methods))
    allocs = Dict(m => Dict(o => Int[]     for o in orders) for m in keys(methods))

    for d in dims
        println("===== d = $d =====")
        for o in orders
            case_results = benchmark_case(o, n, d; methods = methods)
            for (mname, stats) in case_results
                push!(times[mname][o],  stats.time_ms)
                push!(mems[mname][o],   stats.memory)
                push!(allocs[mname][o], stats.allocs)
            end
        end
    end

    return BenchmarkResults(
        collect(dims),
        orders,
        collect(keys(methods)),
        times,
        mems,
        allocs,
    )
end

# ============================================================
# 6. Averaging + plotting
# ============================================================

function average_over_orders(res::BenchmarkResults)
    avg_times = Dict{Symbol,Vector{Float64}}()
    avg_mem   = Dict{Symbol,Vector{Float64}}()

    for m in res.methods
        avg_times[m] = Float64[]
        avg_mem[m]   = Float64[]

        for i in eachindex(res.dims)
            tvals = [res.times[m][o][i] for o in res.orders]
            mvals = [res.mem[m][o][i]   for o in res.orders]

            push!(avg_times[m], mean(tvals))
            push!(avg_mem[m],   mean(mvals))
        end
    end

    return avg_times, avg_mem
end

function make_plots(res::BenchmarkResults)
    avg_times, avg_mem = average_over_orders(res)

    p_time = plot(
        xlabel = "Tensor Order (d)",
        ylabel = "Average Median Time (ms)",
        title  = "CyclicShift vs TensorToolbox.ttm: Time",
        legend = :topleft,
        yscale = :log10,
    )

    for m in res.methods
        plot!(
            p_time,
            res.dims,
            avg_times[m],
            label = string(m),
            lw = 2,
            marker = :circle,
            markersize = 5,
            markerstrokewidth = 1,
        )
    end


    return plot(p_time, layout = (2, 1), size = (900, 700))
end

# ============================================================
# 7. main
# ============================================================

function main(; n::Int = 2, dims = 3:9, seed::Int = 1234, methods = METHODS)
    Random.seed!(seed)
    res = run_experiments(n, collect(dims); methods = methods)
    fig = make_plots(res)
    display(fig)
    return res
end

main()
